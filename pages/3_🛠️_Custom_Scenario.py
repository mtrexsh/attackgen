...
def generate_scenario_wrapper(openai_api_key, model_name, messages):
    if client is not None:  # If LangChain client has been initialized
        ...
        @traceable(
            ...
            tags=["openai", "custom_scenario"],
            client=client,
        )
        def generate_scenario(
            openai_api_key, model_name, messages, *, run_tree: RunTree
        ):
            ...
            try:
                ...
                st.session_state["run_id"] = str(
                    run_tree.id
                )  # Ensure run_id is updated even on failure
                return None
...
def generate_scenario_azure_wrapper(messages):
    if client is not None:  # LangSmith client has been initialised
        ...
        @traceable(
            run_type="llm",
            name="Custom Scenario (Azure OpenAI)",
            tags=["azure", "custom_scenario"],
            ...
            try:
                ...
                with st.status("Generating scenario...", expanded=True):
                    ...
                    response = llm.chat.completions.create(
                        model=azure_deployment_name, messages=formatted_messages
                    )
                    st.write("Scenario generated successfully.")
                    st.session_state["run_id"] = str(
                        run_tree.id
...
def generate_scenario_google_wrapper(google_api_key, model, messages):
    if client is not None:  # If LangSmith client has been initialised
        ...
        @traceable(
            run_type="llm",
            name="Custom Scenario (Google AI API)",
            tags=["google", "custom_scenario"],
            client=client,
        )
        def generate_scenario_google(
            google_api_key, model, messages, *, run_tree: RunTree
        ):
            try:
                ...
                with st.status("Generating scenario...", expanded=True):
                    ...
                    llm = ChatGoogleGenerativeAI(
                        google_api_key=google_api_key, model=model
                    )
                    st.write("Model initialised. Generating scenario, please wait.")
...
def generate_scenario_mistral_wrapper(mistral_api_key, model_name, messages):
    if client is not None:  # If LangSmith client has been initialised
        ...
        @traceable(
            run_type="llm",
            name="Custom Scenario (Mistral API)",
            tags=["mistral", "custom_scenario"],
            client=client,
        )
        def generate_scenario_mistral(
            mistral_api_key, model_name, messages, *, run_tree: RunTree
        ):
            try:
...
def generate_scenario_groq_wrapper(groq_api_key, model_name, messages):
    if client is not None:  # If LangSmith client has been initialised
        ...
        @traceable(
            run_type="llm",
            name="Custom Scenario (Groq API)",
            tags=["groq", "custom_scenario"],
            client=client,
        )
        def generate_scenario_groq(
            groq_api_key, model_name, messages, *, run_tree: RunTree
        ):
            try:
...
def generate_scenario_ollama_wrapper(model):
    if client is not None:  # If LangSmith client has been initialised
        ...
        @traceable(
            run_type="llm",
            name="Custom Scenario (Ollama)",
            tags=["ollama", "custom_scenario"],
            client=client,
        )
...
def generate_custom_scenario_openai_wrapper(messages):
    ...
    if client is not None:  # If LangChain client has been initialized
        ...
        @traceable(
            run_type="llm",
            name="Custom Scenario (Custom)",
            tags=["custom", "custom_scenario"],
            client=client,
        )
        def generate_scenario_custom(formatted_messages_arg, *, run_tree: RunTree):
            try:
                ...
                with st.status(
                    "Generating scenario with custom model...", expanded=True
                ):
...
st.markdown(
    "# <span style='color: #1DB954;'>Generate Custom Scenario🛠️</span>",
...
st.markdown(
    """
    This tool helps you generate a custom scenario based on ATT&CK techniques. 
    Provide the essential information below, and optionally add more details in the additional sections.
"""
)
...
with st.expander("Use a Template (Optional)"):
    ...
    selected_template = st.selectbox(
        ...
        options=[""]
        + list(
            incident_response_templates[matrix].keys()
        ),  # Add an empty option for no selection
        format_func=lambda x: (
            "Select a template" if x == "" else x
...
if not techniques_df.empty:
    ...
    if matrix == "Enterprise":
        st.info(
            "📝 Techniques are searchable by either their name or technique ID (e.g. `T1556` or `Phishing`)."
        )
    elif matrix == "ICS":
        st.info(
            "📝 Techniques are searchable by either their name or technique ID (e.g. `T0814` or `Denial of Service`)."
        )
    else:
...
context = st.text_area(
    ...
    help="Provide a brief overview of what you're building or assessing",
)

# Design/control decisions
design_decisions = st.text_area(
    "What security decisions or controls have you implemented?",
    help="Describe any security measures, controls, or design decisions you've made",
)

# Security concerns
security_concerns = st.text_area(
    "What are your main security concerns?",
    help="List any specific security concerns or areas you'd like to focus on",
)
...
cloud_providers = st.multiselect("Cloud Providers", ["AWS", "Azure", "GCP", "Other"])

# Security Context
st.markdown("### Security Context")
compliance_requirements = st.multiselect(
    "Compliance Requirements",
    ["GDPR", "HIPAA", "PCI DSS", "SOC 2", "ISO 27001", "NIST", "Other"],
)
...
try:
    if len(selected_techniques) > 0:
        ...
        assessment_request = {
            ...
            "company_info": {"name": company_name, "industry": industry},
            "technical_details": {"cloud_providers": cloud_providers},
            "security_context": {"compliance_requirements": compliance_requirements},
        }

        # Get the model provider and API key
        model_provider = st.session_state.get("chosen_model_provider", "OpenAI API")

        # Create message templates
        system_template = """You are a cybersecurity expert. Your task is to generate a comprehensive incident response testing scenario based on the provided information. The scenario should be realistic, detailed, and focused on testing the organization's incident response capabilities.
        ...
        3. Detection and Response
            - How the attack might be detected
            - Key indicators of compromise
            - Response challenges

        Format your response in clear sections with markdown.
        Focus on creating a realistic and challenging scenario that tests the organization's incident response capabilities."""
        ...
        Please generate a comprehensive incident response testing scenario based on this information."""
        ...
        messages = ChatPromptTemplate.from_messages(
            ...
            cloud_providers=(
                ", ".join(cloud_providers) if cloud_providers else "Not specified"
            ),
            ...
            template_info=(
                f"Selected Techniques: {', '.join(selected_techniques)}"
...
try:
    if model_provider == "Azure OpenAI Service":
        if st.button("Generate Scenario", key="generate_custom_scenario_azure"):
            ...
            if not os.environ["AZURE_OPENAI_ENDPOINT"]:
                st.info(
                    "Please add your Azure OpenAI Service API endpoint to continue."
                )
            if not os.environ["AZURE_DEPLOYMENT"]:
                st.info(
                    "Please add the name of your Azure OpenAI Service Deployment to continue."
                )
            elif not industry:
                ...
                if response is not None:
                    ...
                    st.session_state["custom_scenario_text"] = (
                        custom_scenario_text  # Store the generated scenario in the session state
                    )
                    ...
                    st.session_state["last_scenario_text"] = (
                        custom_scenario_text  # Store the last scenario in the session state for use by the Scenario Assistant
                    )

                else:
                    # If a scenario has been generated previously, display it
                    if (
                        "custom_scenario_text" in st.session_state
                        and st.session_state["custom_scenario_generated"]
                    ):
                        st.markdown("---")
        ...
        if st.button("Generate Scenario", key="generate_custom_scenario_google"):
            ...
            if not os.environ["GOOGLE_MODEL"]:
                ...
                response = generate_scenario_google_wrapper(
                    google_api_key, model_name, messages
                )
                st.markdown("---")
                if response is not None:
                    st.session_state["custom_scenario_generated"] = True
        ...
        if st.button("Generate Scenario", key="generate_custom_scenario_mistral"):
            ...
            if not os.environ["MISTRAL_MODEL"]:
                ...
                response = generate_scenario_mistral_wrapper(
                    mistral_api_key, model_name, messages
                )
                st.markdown("---")
                if response is not None:
                    st.session_state["custom_scenario_generated"] = True
        ...
        if st.button("Generate Scenario", key="generate_custom_scenario_groq"):
            ...
            if not os.environ["GROQ_MODEL"]:
                ...
                response = generate_scenario_groq_wrapper(
                    groq_api_key, model_name, messages
                )
                st.markdown("---")
                if response is not None:
                    st.session_state["custom_scenario_generated"] = True
                    ...
                    custom_scenario_text = re.sub(
                        r"^```\w*\n|```$", "", custom_scenario_text, flags=re.MULTILINE
                    ).strip()
        ...
        if st.button("Generate Scenario", key="generate_custom_scenario_custom"):
            ...
            if not st.session_state.get("custom_base_url"):
                ...
                if response is not None:
                        ...
                        if hasattr(response, "choices") and response.choices:
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    st.error(
                                        "Error processing response: 'content' attribute missing from message."
                                    )
                                    st.json(response.model_dump_json(indent=2))
                            else:
                                st.error(
                                    "Error processing response: 'message' attribute missing from the first choice."
                                )
                                st.json(response.model_dump_json(indent=2))
                        else:
                            st.error(
                                "Error processing response: 'choices' list is missing or empty."
                            )
                            st.json(response.model_dump_json(indent=2))
                    except Exception as processing_error:
                        st.error(
                            f"An error occurred while processing the scenario response: {processing_error}"
                        )
                    ...
                    # Response was None
                    st.warning(
                        "Scenario generation failed. Check the error message above."
                    )
                    if (
                        "custom_scenario_text" in st.session_state
                        and st.session_state["custom_scenario_generated"]
        ...
        if st.button("Generate Scenario", key="generate_custom_scenario"):
            ...
            if not model_name:
                ...
                # Generate a scenario
                response = generate_scenario_wrapper(
                    openai_api_key, model_name, messages
                )
                st.markdown("---")
                if response is not None:
                    st.session_state["custom_scenario_generated"] = True
    ...
    if "LANGCHAIN_API_KEY" not in st.secrets:
        st.info(
            "ℹ️ No LangChain API key has been set. This run will not be logged to LangSmith."
        )
                ...
                try:
                    ...
                    if run_id:
                        ...
                        feedback_placeholder.warning(
                            "No run ID found. Please generate a scenario first."
                        )
                except Exception as e:
                    # Update the feedback message in the placeholder
                    feedback_placeholder.error(
                        f"An error occurred while creating feedback: {str(e)}"
                    )
